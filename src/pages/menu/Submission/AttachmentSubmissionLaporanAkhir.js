...
const AttachmentSubmission = forwardRef(({ onFilesChange }, ref) => {
    ...
    const processFiles = async () => {
      ...
      for (const file of selectedFiles) {
        ...
        if (!isPDF) {
          ...
          errorMessage = `${file.name} exceeds the maximum size of 5 MB.`;
          break;
        } else {
          try {
      ...
      >
        ...
        <Typography variant="subtitle1" sx={{ marginBottom: 1 }}>
          {files.length > 0 ? (
            ...
            >
              Anda telah mengupload sebuah berkas, tolong tekan tombol upload.
            </label>
          ) : (
            <label
              htmlFor="multi-file-input"
        ...
        <Typography variant="body2" color="#2E263DB2">
          Hanya dapat mengupload berkas PDF dengan batasan kurang dari 5 MB.
        </Typography>
